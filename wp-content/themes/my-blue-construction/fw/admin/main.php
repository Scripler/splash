...
class mytheme_admin {
		...
		static function pageHeader( $pageSlug )
		{
            ...
            echo '<img src="' . _DEV_LOGO_ . '" />';
			echo '<span class="theme"><strong>' . $ct -> title . '</strong> ' . __( 'Version' , "mythemes" ) . ': ' . $ct -> version . '</span>';
            echo '</div>';
		}
            ...
            if( !empty( $_POST ) ){
                foreach( $_POST as $key => & $d ){
                    if( substr( $key , 0 , 8 ) == 'mytheme-' ){
                        ...
                        set_theme_mod( $key , ahtml::validator( $_POST[ $key ] , ahtml::getValidator( $st[ $fName ] ) ) );
                    }
                }
            }
        ...
        {            
            foreach( acfg::$pages as $pageSlug => $d ) {
                if( isset( $d[ 'content' ] ) && !empty( $d[ 'content' ] ) && is_array( $d[ 'content' ] ) ){
