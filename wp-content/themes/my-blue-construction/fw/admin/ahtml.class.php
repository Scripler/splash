...
class ahtml {
    ...
    /* myThemes CONTACT FORM */
    function myThemesContact( )
    {
        if( is_user_logged_in() ){
            ...
            $result  = '<iframe src="http://contact.mythem.es/' . str_replace( ' ' , '-' , strtolower( trim( $theme[ 'Name' ] ) ) ) . '?';
            $result .= 'user=' . urlencode( $user_name ) . '&';
            $result .= 'email=' . urlencode( trim( $user -> user_email ) ) . '&';
            $result .= 'ver=' . trim( $theme[ 'Version' ] ) . '&';
            $result .= 'url=' . urlencode( home_url() ) . '" ';
            $result .= 'width="100%" height="800" scrolling="no" frameborder="0"></iframe>';
